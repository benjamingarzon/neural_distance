...
from util import euclidean_distance, plot_training, create_pairs, \
    create_triplets, plot_embedding, balance_labels, get_correct, metrics, \
    squeeze_dist_diff
...
class Net():
    ...
    def build(self, input_shape):
        ...
        x = BatchNormalization(name = 'batch_norm1')(inputs)
        
        x = Dense(self.params['n1'], 
                  name = 'dense1',
                  activation = self.params['activation'])(x)
...
#        x = BatchNormalization()(x)
    ...
    def fit(self, X_tuples_train, label_train, X_tuples_test = None, 
            label_test = None, plot = True):

        if self.model_ref is not None and self.save_model:            
        ...
        if label_test is not None:
            ...
            X_tuples_train, label_train,
            	validation_data=(X_tuples_test, label_test),
    ...
    def fit_file(self, train_file, test_file = None):         
        ...
        index_train, X_tuples_train, label_train = self.create_tuples(X_train, y_train)
        ...
        if test_file is not None:
            ...
            index_test, X_tuples_test, label_test = self.create_tuples(X_test, y_test)
        else:
            X_tuples_test, label_test = None, None
        
        self.fit(X_tuples_train, label_train, X_tuples_test, label_test)
        self.trained = True
    ...
    def fit_and_predict_file_cv(self, data_file):         
        ...
        if self.grouping_variable is None:
            ...
            grouping_labels = data[self.grouping_variable].values
        ...
        for i, (train_index, test_index) in enumerate(folds.split()):
            ...
            score = self.metrics_function(embeddings, X_test, y_test, 
                                          grouping_labels)
            scores.append(score)
            ...
            try:
                ...
                self.log('Could not plot embedings: %s'%(plot_file))
    ...
    def predict_file(self, test_file):
        ...
        try:
            ...
            self.log('Could not plot embedings: %s'%(plot_file))
        ll, _ = pd.factorize(y_test)
        np.savetxt(plot_file + '-meta.csv', ll.astype(int), fmt = '%d')
        np.savetxt(plot_file + '-vec.csv', embeddings, delimiter="\t")
        score = self.metrics_function(embeddings, X_test, y_test,
                                      grouping_labels)
        return(score)        
...
class TripletNet(Net):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.create_tuples = create_triplets
    ...
    def build(self, input_shape):
        ...
        X_anc = Input(shape=input_shape, name='input_anc')
        X_pos = Input(shape=input_shape, name='input_pos')
        X_neg = Input(shape=input_shape, name='input_neg')

        emb_anc = self.sister(X_anc)
        emb_pos = self.sister(X_pos)
        emb_neg = self.sister(X_neg)
    
        outputs = Lambda(squeeze_dist_diff, name = 'squeeze_dist_diff')([emb_anc, emb_pos, emb_neg])
...
class SiameseNet(Net):
    ...
    def __init__(self, *args, **kwargs):
        ...
        super().__init__(*args, **kwargs)
        self.create_tuples = create_pairs
        if self.params is None:
